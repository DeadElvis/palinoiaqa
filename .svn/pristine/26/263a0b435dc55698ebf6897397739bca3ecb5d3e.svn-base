    ...
    public class BusinessRulesDAL
    {
        ...
        public string addBusinessRule(viewBusinessRule vBusinessRule)
        {
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                try
                {
                    BusinessRule newBR = new BusinessRule();
                    newBR.ID = 0;
                    newBR.Name = vBusinessRule.Name;
                    newBR.fk_StatusID = (int)vBusinessRule.StatusID;
                    newBR.Text = vBusinessRule.Text;
                    newBR.Active = vBusinessRule.Active;
                    newBR.fk_SectionID = (int)vBusinessRule.SectionID;
                    newBR.UpdatedBy = (int)vBusinessRule.UpdatedBy;
                    //newBR.lkup_Status.Text = vBusinessRule.Status;
                    newBR.Active = vBusinessRule.Active;
                    newBR.fk_SectionID = vBusinessRule.SectionID;
                    context.BusinessRules.Add(newBR);
                    ...
                    {
                        context.BusinessRule_Sections.Remove(deleteBRSection);
                    }
                    //add business rule section relationship
                    var brSection = new BusinessRule_Sections();
                    brSection.ID = 0;
                    brSection.fk_BusinessRuleID = vBusinessRule.ID;
                    brSection.fk_SectionID = vBusinessRule.SectionID;
                    context.BusinessRule_Sections.Add(brSection);
                    context.SaveChanges();
        ...
        public string deleteBusinessRule(int deleteID, int userID)
        {
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                try
                {
                    ...
                    context.BusinessRules.Remove(deleteBusinessRule);
                    context.SaveChanges();
                    result = "OK";
                }
                catch (Exception ex)
