    ...
    public class CustomerServiceMessagesDAL
    {
        ...
        public string addCSM(viewCustomerServiceMessage vCSM)
        {
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                try
                {
                    CustomerServiceMessage newCSM = new CustomerServiceMessage();
                    newCSM.ID = 0;
                    newCSM.Name = vCSM.Name;
                    newCSM.fk_StatusID = vCSM.StatusID;
                    newCSM.fk_CSMTypeID = vCSM.CSMTypeID;
                    newCSM.fk_CSMResponseTypeID = vCSM.CSMResponse_TypeID;
                    newCSM.Text = vCSM.Text;
                    newCSM.fk_SectionID = vCSM.SectionID;
                    newCSM.Active = vCSM.Active;
                    newCSM.UpdatedBy = vCSM.UpdatedBy;
                    //newBR.lkup_Status.Text = vBusinessRule.Status;
                    newCSM.Active = vCSM.Active;
                    context.CustomerServiceMessages.Add(newCSM);
                    ...
                    {
                        context.CustomerServiceMessage_Sections.Remove(deleteCSMSection);
                    }
                    //add business rule section relationship
                    var csmSection = new CustomerServiceMessage_Sections();
                    csmSection.ID = 0;
                    csmSection.fk_CustomerServiceMessageID = vCSM.ID;
                    csmSection.fk_SectionID = vCSM.SectionID;
                    context.CustomerServiceMessage_Sections.Add(csmSection);
                    context.SaveChanges();
                    result = "OK";
                }
        ...
        public string deleteCSM(int deleteID, int userID)
        {
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                try
                {
                    ...
                    context.CustomerServiceMessages.Remove(deleteCSM);
                    context.SaveChanges();
                    result = "OK";
                }
                catch (Exception ex)
