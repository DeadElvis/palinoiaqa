        ...
        public string deleteDocumentByID(int deleteID)
        {
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                try
                {
                    ...
                    context.Documents.Remove(deleteDocument);
                    context.SaveChanges();
                    ...
                    foreach (var docRelationship in documentRelationships)
                    {
                        ...
                        context.Chapters.Remove(deleteChapter);
                        //delete document chapter relationship
                        context.Document_Chapters.Remove(docRelationship);
                    }
                    context.SaveChanges();
                    result = "OK";
        ...
        public string addNewDocument(viewDocument doc)
        {
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                try
                {
                    ...
                    document.ID = 0;
                    document.fk_DocumentType = (long)doc.DocumentTypeID;
                    document.Name = doc.Name;
                    document.Active = (bool)doc.Active;
                    document.UpdatedBy = (int)doc.UpdatedBy;
        ...
        public void saveDocumentChapterRelationship(int documentID, int chapterID, int seqNum)
        {
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                ...
                if (record == null)
                {
                    ...
                    var newRelationship = new Document_Chapters();
                    newRelationship.ID = 0;
                    newRelationship.SeqNum = (long)seqNum;
                    newRelationship.fk_DocumentID = (long)documentID;
                    newRelationship.fk_ChapterID = (long)chapterID;
                    context.Document_Chapters.Add(newRelationship);
                    context.SaveChanges();
                }
                else
                {
        ...
        public string addNewChapter(int documentID, viewChapter newChapter)
        {
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                try
                {
                    ...
                    chapter.ID = 0;
                    chapter.fk_ChapterTypeID = (long)newChapter.TypeID;
                    chapter.Title = newChapter.Title;
                    chapter.UpdatedBy = (long)newChapter.UpdatedBy;
                    chapter.Active = newChapter.Active;
            ...
            using (var context = palinoiaDAL.getContextForProject())
            {
                try
                {
                    ...
                    if (!imageInDatabase)
                    {
                        ...
                        di.ID = iData.ID;
                        di.FileName = iData.FileName;
                        di.UpdatedBy = iData.UpdatedBy;
                        di.Description = iData.Description;
                        context.DocumentImages.Add(di);
                        context.SaveChanges();
